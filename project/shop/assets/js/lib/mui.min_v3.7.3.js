...
function(a, b, c) {
	...
	// }), b.addEventListener("click", function(b) { //Unable to preventDefault inside passive event listener due to target being treated as passive. See 
	},{passive: false}), b.addEventListener("click", function(b) {
		...
		var e = "mui-off-canvas-left",
			...
			m = a.Class.extend({
				...
				initEvent: function() {
					...
					this.classList.contains("mui-draggable") && (this.wrapper.addEventListener("", this), 
					this.wrapper.addEventListener("drag", this), this.wrapper.addEventListener("dragend", this)), this.wrapper.addEventListener(
	...
	$.showLoading = function(message, type, options) {
		...
		if ($.os.plus && type !== 'div') {
			...
			if (options.loading && options.loading.icon && options.loading.icon.indexOf('.') > -1) {
				...
				html += '<i class="' + (options.loading && options.loading.icon ? options.loading.icon :
					'mui-spinner mui-spinner-white') + '"></i>';
			}
...
/*
	<button onclick="insert()">测试插入</button>
	<button onclick="select()">测试读取</button>
